...
def plot_goal_predictions(acc_df):
    ...
    for j in range(5):
        for i in range(5):
            ...
            # ax.set_title(f'$({true_x}, {true_y})$')

            # draw red border around the true goal position
            width = 2
...
def reproduce_figure():
    data = [
        ...
        for csv_path in sorted(
            Path("figures/state_decoding_22-11-24/goal_predictions").glob("data_*.csv")
        )
    ]
    plot_goal_predictions(*data)
    plt.savefig(
        "figures/state_decoding_22-11-24/goal_predictions/goal_predictions.pdf",
        bbox_inches="tight",
        dpi=1000,
    )
