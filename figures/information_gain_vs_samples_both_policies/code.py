from pathlib import Path
from reproducible_figures.plotting import set_plotting_style
import matplotlib
import matplotlib.pyplot as plt
import pandas as pd
import seaborn as sns


matplotlib.use("pdf")


policy_samples = 10


def plot_information_gain(
    df: pd.DataFrame, wrong_policy_df, stochastic_policy_df, max_gain: float
):
    set_plotting_style(font_scale=2.5)
    plt.figure(figsize=(10, 6))
    ax = sns.lineplot(
        data=df,
        x="k",
        y="gain",
        errorbar=lambda x: (x.min(), x.max()),
        label="Correct Optimal Policy",
    )
    sns.lineplot(
        data=wrong_policy_df,
        x="k",
        y="gain",
        ax=ax,
        errorbar=lambda x: (x.min(), x.max()),
        label="Incorrect Optimal Policy",
    )
    sns.lineplot(
        data=stochastic_policy_df[stochastic_policy_df.policy_samples == 9],
        x="k",
        y="gain",
        ax=ax,
        errorbar=lambda x: (x.min(), x.max()),
        label="10 Randomly Sampled\nOptimal Policies",
    )
    # sns.lineplot(data=stochastic_policy_df[stochastic_policy_df.policy_samples == 9], x='k', y='gain', ax=ax, errorbar=lambda x: (x.min(), x.max()))
    ax.axhline(max_gain, color="black", linestyle="--", label="Max Gain", alpha=0.5)
    ax.set_xlabel("Number of Samples")
    ax.set_ylabel("Information Gain (bits)")
    ax.legend(loc="upper right", bbox_to_anchor=(1.7, 1))


def reproduce_figure():
    data = [
        pd.read_csv(csv_path)
        for csv_path in sorted(Path("figures/information_gain_vs_samples_both_policies").glob(
            "data_*.csv"
        ))
    ]
    plot_information_gain(*data, max_gain=4.643856189774724)
    plt.savefig(
        "figures/information_gain_vs_samples_both_policies/information_gain_vs_samples_both_policies.pdf",
        bbox_inches="tight",
        dpi=1000,
    )


if __name__ == "__main__":
    reproduce_figure()
