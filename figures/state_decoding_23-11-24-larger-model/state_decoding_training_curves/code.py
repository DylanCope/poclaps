...
def reproduce_figure():
    data = [
        ...
        for csv_path in sorted(
            Path(
                "figures/state_decoding_23-11-24-larger-model/state_decoding_training_curves"
            ).glob("data_*.csv")
        )
    ]
    plot_state_decoding_training_curves(*data)
    plt.savefig(
        "figures/state_decoding_23-11-24-larger-model/state_decoding_training_curves/state_decoding_training_curves.pdf",
        bbox_inches="tight",
        dpi=1000,
    )
