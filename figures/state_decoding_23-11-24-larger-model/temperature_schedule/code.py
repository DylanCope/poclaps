...
def reproduce_figure():
    data = [
        ...
        for csv_path in sorted(
            Path(
                "figures/state_decoding_23-11-24-larger-model/temperature_schedule"
            ).glob("data_*.csv")
        )
    ]
    plot_temp_schedules(*data)
    plt.savefig(
        "figures/state_decoding_23-11-24-larger-model/temperature_schedule/temperature_schedule.pdf",
        bbox_inches="tight",
        dpi=1000,
    )
